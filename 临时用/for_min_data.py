...
def main_sh(date):
    ...
    # 文件路径
    order_file_path = rf'\\Desktop-79nue61\sz\{date}_order_sz.feather'
    trade_file_path = rf'\\Desktop-79nue61\sz\{date}_trade_sz.feather'
...
if __name__ == "__main__":
    ...
    all_date_sh = os.listdir(r'\\Desktop-79nue61\sz')
    os.makedirs(r'\\Desktop-79nue61\SZ_min_data_big_order', exist_ok=True)
    ...
    for d in all_date_sh:
        if not os.path.exists(rf'\\DESKTOP-79NUE61\SZ_min_data_big_order\{d}.feather'):
            result = main_sh(d)
            feather.write_dataframe(result, rf'\\DESKTOP-79NUE61\SZ_min_data_big_order\{d}.feather')
