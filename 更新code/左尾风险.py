...
def update(today='20250821'):
    ...
    if os.path.exists(os.path.join(DataPath.save_path_update,'ES_1%.feather')):
        old = feather.read_dataframe(os.path.join(DataPath.save_path_update, 'ES_1%.feather'))
        tar_dl = sorted(list(old.DATE.unique()))[-50:][0]
        object = factor_leftail(start=tar_dl, end=today, window=20)
        # df1,df2,df3,df4=object.cal()
        df1=object.cal()
        for file_name,data in {'ES_1%':df1}.items():
            file_name+='.feather'
