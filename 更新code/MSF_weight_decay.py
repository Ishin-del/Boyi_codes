...
class MSF_half(object):
    def __init__(self, start='20200101', end='20221231', path=None, savepath=None, halflife=10, test_path=None):
        # self.testpath = DataPath.out_path
    ...
    def __daily_calcuation(self):
        if os.path.exists(self.tmp_path + '\\basic_data.feather'):
            df_old = feather.read_dataframe(self.tmp_path + '\\basic_data.feather')
            exist_date = []
        ...
        newdata=Parallel(n_jobs=10)(delayed(get_daily)(i) for i in tqdm(date_list))
        if newdata:
            ...
            df_old.to_feather(self.tmp_path + '\\basic_data.feather')
    ...
    def cal_MSF_half(self):
        def dataframe_ewm_rolling(dataframe: pd.DataFrame, halflife: int = 1, min_periods: int = 1):
            ...
            warnings.filterwarnings('ignore')
            numeric_df = dataframe._get_numeric_data() # 转成数字格式？
        ...
        data = feather.read_dataframe(self.tmp_path + '\\basic_data.feather')
        ...
        half = self.halflife
        if os.path.exists(self.savepath + f'\\MSF_half{half}.feather'):
            dz = pd.read_feather(self.savepath + f'\\MSF_half{half}.feather')
        ...
        if os.path.exists(self.savepath + f'\\MSF_half{half}.feather'):
            print(f'MSF_half{half}因子更新中')
            dtz = pd.read_feather(self.savepath + f'\\MSF_half{half}.feather')
            ...
            feather.write_dataframe(data_, DataPath.save_path_update + f'\\MSF_half{half}.feather')
            feather.write_dataframe(data_, self.savepath + f'\\MSF_half{half}.feather')
            print(data_)
        else:
            print(data)
            feather.write_dataframe(data, self.savepath + f'\\MSF_half{half}.feather')
            feather.write_dataframe(data, DataPath.save_path_update + f'\\MSF_half{half}.feather')
            # print(data)
...
def update(today='20250820'):
    MSF_object = MSF_half(start='20220631', end=today, savepath=DataPath.factor_out_path)
    MSF_object.run()

if __name__ == '__main__':
    update()
    # MSF_object = MSF_half(start='20200101', end='20221231', savepath=DataPath.factor_out_path)
    # MSF_object.run()