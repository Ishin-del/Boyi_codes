...
class factor_outside(object):
    ...
    def helper(self,date):
        ...
        min=min[min['min']>930]
        min=min[(min['min']<1457)].drop_duplicates()
        ...
        daily_nodivided = (daily_nodivided_corr.sum(axis=1) - 1)/30

        # daily_nodivided = (daily_nodivided.corr().abs().sum(axis=1) - 1)/daily_nodivided.shape[1]
        daily_nodivided = pd.DataFrame(daily_nodivided.rename('孤雁出群v2'))
    ...
    def cal(self):
        ...
        if os.path.exists(self.tmp_path + r'\孤雁出群v2.feather'):
            # 检测下是否之前计算过该因子，如果有，就只再计算新多出来的这些天
            df_old = feather.read_dataframe(self.tmp_path + r'\孤雁出群v2.feather')
        ...
        daily_nodivided=Parallel(n_jobs=8)(delayed(self.helper)(date) for date in tqdm(date_list, desc='calculating daily'))
        ...
        if date_list.any():
            ...
            feather.write_dataframe(df_old, self.tmp_path + f'\\孤雁出群v2.feather')
            print(f'孤雁出群v2 has been saved')
        # return df_old

    def month(self):
        data_gu = feather.read_dataframe(self.tmp_path + '\\孤雁出群v2.feather')
        data_gu = data_gu[(data_gu['DATE'] >= self.start) & (data_gu['DATE'] <= self.end)]
        data_gu['20日均孤雁出群v2'] = data_gu.groupby('TICKER').apply(lambda x:
                                 x['孤雁出群v2'].rolling(20, min_periods=20).mean()).reset_index(level = 'TICKER',drop=True).values
        data_gu['20日稳孤雁出群v2'] = data_gu.groupby('TICKER').apply(lambda x:
                                 x['孤雁出群v2'].rolling(20, min_periods=20).std()).reset_index(level = 'TICKER',drop=True).values

        data_gu['20日孤雁出群v2'] = (data_gu['20日稳孤雁出群v2'] + data_gu['20日均孤雁出群v2'])/2
        Fct_list = ['20日稳孤雁出群v2', '20日均孤雁出群v2', '20日孤雁出群v2']
