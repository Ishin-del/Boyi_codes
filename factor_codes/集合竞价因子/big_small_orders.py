...
class Traction_LUD:
    ...
    def __daily_calculation(self):
        ...
        def get_daily(date):
            ...
            if not os.path.exists(os.path.join(self.df_path, filename)):
                ...
                dt['active_flow_dir']=np.where((dt['Side']=='1')&(dt['当前撮合价格']>dt['上一笔撮合价格']),1,
                                    np.where((dt['Side']=='2')&(dt['当前撮合价格']<dt['上一笔撮合价格']),-1,np.nan))
                # 1代表主动流入，-1代表主动流出
                ...
                tmp=dt.groupby(['TICKER','time_range','大小单'])['amount'].sum().reset_index()
                tmp=tmp.pivot(index=['TICKER', 'time_range'], columns='大小单', values='amount').reset_index().rename(columns={'time_range':'time'})
        ...
        # feather.write_dataframe(alls, os.path.join(self.tmp_path, 'acu.feather'))
        # print(alls)

    def cal(self):
        warnings.filterwarnings('ignore')
        df = feather.read_dataframe(os.path.join(self.tmp_path, 'acu.feather'))
