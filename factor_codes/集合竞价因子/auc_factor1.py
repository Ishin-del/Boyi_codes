...
class Auc_fac:
    ...
    def __init__(self, start, end, calender_path=None, df_path=None, savepath=None, daily_path=None, citi_path=None,
        ...
        # self.daily_df = feather.read_dataframe(self.daily_path)[['DATE', 'TICKER', 'amount']]
        # self.daily_df.sort_values(['TICKER', 'DATE'], inplace=True)
        # self.daily_df['pre_amount'] = self.daily_df.groupby('TICKER')['amount'].shift(1)  # close数据用前一天的
        # # self.daily_df['pre_close'] = self.daily_df['close']
        # self.daily_df = self.daily_df[self.daily_df['DATE'] > '20191229'][['DATE', 'TICKER', 'pre_amount']]
    ...
    def __daily_calculation(self):
        ...
        def get_daily(date):
            ...
            if not os.path.exists(os.path.join(self.df_path, filename)):
                ...
                # ----------------------------------------------------
                # head10=dt.groupby('TICKER').head(10)
                # tail10=dt.groupby('TICKER').tail(10)
                # dt=pd.concat([head10,tail10]).reset_index(drop=True)
                # volume_mean_std=dt.groupby('TICKER').agg({'主动净流入额':['mean','std']})
                # volume_mean_std.columns=['主动净流入额_mean','主动净流入额_std']
                # volume_mean_std['threshold']=volume_mean_std['主动净流入额_mean']+volume_mean_std['主动净流入额_std']
                # volume_mean_std=volume_mean_std.reset_index()[['TICKER','threshold']]
                # dt=dt.merge(volume_mean_std,on='TICKER',how='inner')
                # # todo:
                # tmp=dt[dt['主动净流入额']>=dt['threshold']][['TICKER','time']].rename(columns={'time':'tar_start'})
                # tmp=tmp.groupby('TICKER').head(1)
                # tmp['tar_end']=tmp['tar_start']+5
                # dt=dt.merge(tmp,on=['TICKER'],how='left')
                # dt=dt[(dt['time']>dt['tar_start'])&(dt['time']<=dt['tar_end'])]
                # print(dt)
                res=dt.groupby('TICKER').agg({'主动净流入额':'mean'}).reset_index()
                res.columns=['TICKER','主动净流入额mean']
                res['DATE']=date
                return res
        ...
        alls['amt_ratio']=alls['主动净流入额mean']/alls['pre_amount']
        alls.drop(columns=['主动净流入额mean','pre_amount'],inplace=True)
