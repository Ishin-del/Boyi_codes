    ...
    def __daily_calculation(self):
        ...
        def get_daily(date):
            ...
            if not os.path.exists(os.path.join(self.df_path, filename)):
                ...
                dt.sort_values(['TICKER'], inplace=True)
                ...
                dt['sector_code'] = (1 - dt['sec_percent']) * dt['code_percent']
                dt['code_over_mkt'] = (dt['sec_percent'] + dt['code_percent']) / dt['code_percent_mkt']
                dt['code_sec_ratio'] = dt['code_percent_mkt'] / dt['sec_percent']
                dt['comp_fac']=dt['sector_code']+dt['code_over_mkt']+dt['code_sec_ratio']

                pre_ret = dt[['TICKER', 'auc_ret']].drop_duplicates().reset_index(drop=True)
                ret_df1=dt[['TICKER','sector_code']].drop_duplicates().reset_index(drop=True)
                # -------------------------------------------
                # dt=dt.merge(self.citi_df,on=['TICKER','DATE'],how='inner')
                # tickers = dt['TICKER'].unique()
                ...
                tmp_dict1=ret_df1.set_index('TICKER')['sector_code'].to_dict()
                tmp_ret1 = tmp_codes.applymap(lambda x: tmp_dict1.get(x))
                res1 = (tmp_values * tmp_ret1).sum() / tmp_values.sum()
                res1 = pd.DataFrame(res1).reset_index()
                res1 = res1[['TICKER', 0]].rename(columns={0: 'exp_LUD1'})
                # ------------------------------------------------------
                res=res.merge(res1,on='TICKER',how='inner')
                res['DATE'] = date
                res=res.merge(pre_ret,on='TICKER',how='inner')
                res['traction_LUD']=res['auc_ret']-res['exp_LUD1']
                res.drop(columns='auc_ret',inplace=True)
                res = res[['TICKER', 'DATE', 'traction_LUD']].merge(
                    dt[['TICKER', 'DATE', 'sec_code_percent','sector_code','code_over_mkt','code_sec_ratio','comp_fac']], \
