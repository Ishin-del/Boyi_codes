...
def cal(date):
    ...
    # df['act_diff']=df['active_buy_volume']-df['active_sell_volume'] #todo:这是第一版
    df['act_diff']=df.groupby('TICKER')['active_buy_volume'].diff() #todo: 这是第三版
    tmp1=df.groupby('TICKER')['min_ret'].mean()
    # 最高排名10%，20%
    df.sort_values(['TICKER','act_diff'],ascending=[True,False],inplace=True)
    res1=help(df,100,tmp1,'retOnVol_top_24') #24 #todo:top_num的变化是第二版
    res2=help(df,150,tmp1,'retOnVol_top_47') #47
    # 最低排名10%，20%
    df.sort_values(['TICKER','act_diff'],inplace=True)
    res3=help(df,100,tmp1,'retOnVol_bottom_24')
    res4=help(df,150,tmp1,'retOnVol_bottom_47')
...
def run(start,end):
    ...
    res = process_na_stock(res, 'retOnVol_top_24')
    res['retOnVol_top_24_roll20'] = res.groupby('TICKER')['retOnVol_top_24'].rolling(20, 5).mean().values
    res['retOnVol_top_47_roll20'] = res.groupby('TICKER')['retOnVol_top_47'].rolling(20, 5).mean().values
    res['retOnVol_bottom_24_roll20'] = res.groupby('TICKER')['retOnVol_bottom_24'].rolling(20, 5).mean().values
    res['retOnVol_bottom_47_roll20'] = res.groupby('TICKER')['retOnVol_bottom_47'].rolling(20, 5).mean().values
    res.replace([np.inf, -np.inf], np.nan, inplace=True)
    res=res[['DATE','TICKER','retOnVol_top_24_roll20','retOnVol_top_47_roll20','retOnVol_bottom_24_roll20','retOnVol_bottom_47_roll20']]
    return [res]
def update(today):
    update_muli('retOnVol_top_24_roll20.feather',today,run)
