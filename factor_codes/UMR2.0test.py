...
def cal(date):
    ...
    # tmp1=df.groupby('TICKER')['act_diff_self'].mean()
    # df.sort_values(['TICKER','active_buy_volume'],ascending=[True,False],inplace=True)
    # df=df.groupby('TICKER').head(10) # 选变化最多的10分钟
    # tmp2=df.groupby('TICKER')['act_diff_self'].mean()
    # # tmp=tmp1/tmp2
    # tmp=tmp1/tmp2
    # tmp=tmp.reset_index().rename(columns={'act_diff_self':'risk'})
    #-----------------------
    #way2
    # tmp1=df.groupby('TICKER')['min_ret'].std()#.reset_index().rename(columns={'min_ret':'risk'})
    # tmp2=df.groupby('TICKER')['min_ret'].mean()
    # tmp=tmp2+tmp1
    # tmp=tmp.reset_index().rename(columns={'min_ret':'risk'})
    # -----------------------
    # df['volume_volatility'] = df.groupby(['TICKER'])['volume'].std() /df.groupby(['TICKER'])['volume'].mean()
    # df['volume_price_divergence'] = abs(df['min_ret'] * df['volume'])
    # tmp= df.groupby('TICKER')['volume_price_divergence'].std().reset_index()

    # way6
    df['volume_pct'] = df.groupby('TICKER')['volume'].apply(lambda x:100*x/x.sum()).reset_index(drop=True)
    df['weighted_ret_by_volpct'] = df['min_ret']*df['volume_pct']
    tmp = df.groupby('TICKER')['weighted_ret_by_volpct'].std().reset_index(drop=False)

    # tset:
    # tmp=df[['TICKER']].drop_duplicates()
    # tmp['risk']=1
    tmp['DATE']=date
...
def run(start,end):
    ...
    res=process_na_stock(res,'risk')
    res.sort_values(['TICKER', 'DATE'], inplace=True)
    # t=21, 计算过去20天均值
    # res['adjust']=res.groupby('TICKER')['risk'].rolling(20,5).mean().values
    res['adjust']=res.groupby('TICKER')['risk'].transform(lambda x: x.rolling(20, 5).mean())
    res['risk_adjust']=res['adjust']-res['risk']
    ...
    df['factor']=df['risk_adjust']*(df['daily_ret']-df['mkt_ret']) #np.abs()
    df['factor_adjust_beta6'] = df.groupby('TICKER')['factor'].rolling(20, 5).apply(weighted_dot,
                                                            raw=False).reset_index(level=0,drop=True)
    df=df[['DATE','TICKER','factor_adjust_beta6']]
    return [df]
