...
def order_tick_data(date,label='trade'):
    # print('1')
    try:
        sh_df=feather.read_dataframe(os.path.join(DataPath.feather_sh,f'{date}_{label}_sz.feather'))
...
def get_tick_data(date):
    ...
    if os.path.exists(os.path.join(DataPath.tmp_path,date)):
        # print('1')
        fac1=feather.read_dataframe(os.path.join(DataPath.tmp_path,date,'fac1_min.feather'))
        fac2=feather.read_dataframe(os.path.join(DataPath.tmp_path,date,'fac2_min.feather'))
        return fac1,fac2
    ...
    for tmp in res_order:
        ...
        # 选前100
        tmp.sort_values(['TICKER', 'amount'], ascending=[True, True], inplace=True)
    ...
    feather.write_dataframe(fac1,os.path.join(DataPath.tmp_path,date,'fac1_min.feather'))
    feather.write_dataframe(fac2,os.path.join(DataPath.tmp_path,date,'fac2_min.feather'))
    return fac1,fac2
...
def run(start, end):
    ...
    res1.rename(columns={'buy_amount':'有效100s买入额_min','buy_amount_roll20':'有效100s买入额_roll20_min'},inplace=True)
    res2.rename(columns={'sell_amount':'有效100s卖出额_min','sell_amount_roll20':'有效100s卖出额_roll20_min'},inplace=True)
    # print(res1)
    # print(res2)
...
def update(today='20250919'):
    update_muli('有效100s买入额_min.feather',today,run)
