...
def update_muli(filename,today,run):
    ...
    # if False:
        ...
        print('因子生成')
        df1=run(start='20200101',end='20221231')
...
def read_min_data(date):
    ...
    df=df[['DATE','TICKER','min','volume','close','open']]
    ...
    #min_ret为基础
    # 最低10%
    # df.sort_values(['TICKER','min_ret'],inplace=True)
    # res1 = help(df, 24, tmp1, 'ret_bottom24_ratio')
    # res2 = help(df, 47, tmp1, 'ret_bottom47_ratio')
    # ----------------------------------------------------------------------------
    df.sort_values(['TICKER', 'min_ret'], ascending=[True, False], inplace=True)
    # 最高10%
    res = help(df, 24, tmp1, 'ret_top24_ratio')
    # res4 = help(df, 47, tmp1, 'ret_top47_ratio')
    ...
    return res[['DATE','TICKER','ret_top24_ratio']]#, 'ret_top47_ratio', 'ret_bottom24_ratio','ret_bottom47_ratio']]
...
def run(start, end):
    ...
    res=process_na_stock(res,'ret_top24_ratio')
    # res['ret_bottom24_ratio_roll20']=res.groupby('TICKER')['ret_bottom24_ratio'].rolling(20,5).mean().values
    # res['ret_bottom47_ratio_roll20']=res.groupby('TICKER')['ret_bottom24_ratio'].rolling(20,5).mean().values
    res['ret_top24_ratio_roll20']=res.groupby('TICKER')['ret_top24_ratio'].rolling(20,5).mean().values
    # res['ret_top47_ratio_roll20'] = res.groupby('TICKER')['ret_top47_ratio'].rolling(20, 5).mean().values
    res.replace([np.inf, -np.inf], np.nan, inplace=True)
    ...
    res=res[['DATE','TICKER','ret_top24_ratio_roll20']]#, 'ret_top47_ratio_roll20', 'ret_bottom24_ratio_roll20','ret_bottom47_ratio_roll20']]
    return [res]

def update(today='20251010'):
    update_muli('ret_top24_ratio_roll20.feather',today,run)
