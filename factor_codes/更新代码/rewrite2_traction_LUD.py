...
from tool_tyx.tyx_funcs import get_tar_date,process_na_stock
...
def read_min_data(date):
    try:
        ...
        tmp_sz=feather.read_dataframe(os.path.join(r'\\DESKTOP-79NUE61\SZ_min_data_big_order',date+'.feather'))[['min','TICKER','trade_1_num','trade_5_num']]
        df_sz=df_sz.merge(tmp_sz,on=['min','TICKER'],how='inner')
    except FileNotFoundError:
        df_sz=pd.DataFrame()
    try:
        df_sh=feather.read_dataframe(os.path.join(DataPath.sh_min,date+'.feather'))
        tmp_sh = feather.read_dataframe(os.path.join(r'\\DESKTOP-79NUE61\SH_min_data_big_order', date + '.feather'))[
            ['min', 'TICKER', 'trade_1_num', 'trade_5_num']]
        df_sh = df_sh.merge(tmp_sh, on=['min', 'TICKER'], how='inner')
    except FileNotFoundError:
        df_sh=pd.DataFrame()
...
def cal(date,ret_df):
    ...
    df=get_data(date)#[['DATE','TICKER','min','min_ret','close','amount','active_buy_amount']]
    # df=df.merge(ret_df,on='TICKER',how='inner')

    # todo:
    df['trade_5_num_median']=df.groupby('min')['trade_5_num'].transform('median')
    df['dir_up'] = np.where(df['trade_5_num'] > df['trade_5_num_median'], 1, 0)
    df['dir_down'] = np.where(df['trade_5_num'] < df['trade_5_num_median'], 1, 0)
    # -----------------------------------------------------------------
    ...
    res['DATE']=date
    res=res[['DATE','TICKER',0]].rename(columns={0:'exp_LUD_amount'})
    return res
...
def neutralize_factor(df, factor_col, mv_col, industry_col):
    ...
    data['Traction_LUD_trade_num']= model.resid
...
def run(start, end):
    ...
    #     res.append(df)
    res=Parallel(n_jobs=8)(delayed(cal)(date,daily_df[daily_df.DATE==date][['TICKER','ret_roll20']]) for date in tqdm(tar_date))
    ...
    # 做截面反转、市值、行业中性化
    res= res.groupby('DATE').apply(lambda x: neutralize_factor(x, 'exp_LUD_amount', 'float_mv', 'citic_code')).reset_index(drop=True)
    # 重命名列
...
def update_muli(filename,today,run,num=-50):
    ...
    # if False:
        ...
        res=run(start='20200101',end='20251010')
...
def update(today='20251009'):
    update_muli('Traction_LUD_trade_num.feather',today,run)

if __name__=='__main__':
    # read_min_data('20250102')
    # run('20201104','20201105')
    # run('20220101','20250801')
    update('20251010')