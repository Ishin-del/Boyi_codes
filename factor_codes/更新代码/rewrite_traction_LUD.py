...
def cal(date,ret_df):
    ...
    # todo:
    # df['active_buy_amount_median']=df.groupby('min')['active_buy_amount'].transform('median')
    # df['dir_up'] = np.where(df['active_buy_amount'] > df['active_buy_amount_median'], 1, 0)
    # df['dir_down'] = np.where(df['active_buy_amount'] < df['active_buy_amount_median'], 1, 0)

    df['amount_median']=df.groupby('min')['amount'].transform('median')
    df['dir_up'] = np.where(df['amount'] > df['amount_median'], 1, 0)
    df['dir_down'] = np.where(df['amount'] < df['amount_median'], 1, 0)
...
def neutralize_factor(df, factor_col, mv_col, industry_col):
    ...
    data['Traction_LUD_amount']= model.resid
...
def run(start, end):
    ...
    res = res[['DATE', 'TICKER','Traction_LUD_amount']] #, 'exp_LUD_amount',
    return [res]
