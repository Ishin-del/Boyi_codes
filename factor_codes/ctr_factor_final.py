...
def update(end_day='20250904'):
    # 拉取数据：
    save_path = DataPath.save_path_update
    save_path1 = DataPath.save_path_old
    flag = True
    while flag:
        if os.path.exists(os.path.join(save_path, 'ctr.feather')):
            ...
            old_time_list = sorted(old_df.DATE.drop_duplicates().to_list())
            # new_start=old_time_list[max(old_time_list.index(old_last_day)-20*3-5,0)]
            new_start = old_time_list[-50:][0]
            df, full_dl = read_data(time_start=new_start, time_end=end_day)
            OvernightSmart20, ctr = cal(df, full_dl) #, jump_ctr
            for file, data in {'OvernightSmart20.feather': OvernightSmart20, 'ctr.feather': ctr}.items(): #'jump_ctr.feather': jump_ctr
                old_df = feather.read_dataframe(os.path.join(save_path, file))
                # test_df = old_df[old_df.DATE.isin(test_date)].merge(data[data.DATE.isin(test_date)],on=['DATE', 'TICKER'],how='inner').dropna()
                test_df=old_df.merge(old_df,on=['DATE','TICKER'],how='inner').dropna()
                ...
                # test_df=test_df.groupby('TICKER').tail(5)
                if np.isclose(test_df.iloc[:, 2], test_df.iloc[:, 3]).all():
                    ...
                    # feather.write_dataframe(update_df, os.path.join(save_path1, file))
                    feather.write_dataframe(update_df,os.path.join(DataPath.factor_out_path,file))
                else:
                    print('检查更新，数据出问题！')
                    exit()
            flag = False
