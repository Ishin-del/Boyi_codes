...
class Factor_Test():
    ...
    def load_factor_data(self, factor_path, special_condition):
        """
        factor_path 因子路径
        ...
        if special_condition:
            all_data = pd.merge(factor, price_Data, how='left', on=['TICKER', 'DATE']).reset_index(drop=True)
        else:
...
if __name__ == '__main__':
    ...
    info_list = get_Multi_info('请输入测试参数',
                               [('单选', '因子是否当天生成当天使用', ['是', '否']),
                                ...
                                ('单选', '是否使用全市场股票', ['是', '否']),('单选', '因子分箱数', ['5', '10']), ])

    param_list = getMultiInput('请输入参数',
                               ['因子测试起始日期(默认20220104)', '因子测试结束日期(默认20250101)', '因子文件路径',
                                '因子目标未来天数'])
    today_factor, researcher, price_anchor, use_all_market_or_not,bin_split = info_list
    start_Date, den_Date, factors_path, aim_Date = param_list

    # 转换参数类型
    use_type = 'tomorrow' if today_factor == '否' else 'today'
    special_condition = True if use_all_market_or_not == '否' else False
    aim_Date = int(aim_Date)
    bin_split = int(bin_split)
    start_date = start_Date or '20220104'
    end_date = den_Date or '20250101'
    ...
    for factor_path in ls:
        try:
            ...
            plt_save_path = rf'\\Desktop-79nue61\因子测试结果\{researcher}\{today}_{aim_Date}\{factor_path[:-8]}'
