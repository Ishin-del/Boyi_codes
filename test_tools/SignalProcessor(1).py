...
class ProcessFactor:
    def __init__(self, factor_list, data_path=None, list_date_limit=90, start=None, end=None):
        ...
        self.factor_name_ls = factor_list
    ...
    def load_data(self, df_raw_data, ST=False):
        ...
        df_raw_data = df_raw_data[['TICKER', 'DATE'] + self.factor_name_ls]
        ...
        df_raw_data = pd.merge(df_raw_data, citic, on=['TICKER', 'DATE'], how='left')
        df_raw_data = pd.merge(df_raw_data, cap_value, on=['TICKER', 'DATE'], how='left')
        df_raw_data['mv'] = df_raw_data.groupby('TICKER')['mv'].ffill()

        if ST:
            df_st = feather.read_dataframe(os.path.join(self.data_path, 'ST_info.feather'),
                                           columns=['TICKER', 'DATE', 'ST_status'])
            df_raw_data = pd.merge(df_raw_data, df_st, on=['TICKER', 'DATE'], how='left')
            df_raw_data = df_raw_data[df_raw_data['ST_status'] != '是']
        else:
            pass
        ...
        df_raw_data = df_raw_data[(df_raw_data['DATE'] <= self.end) & (df_raw_data['DATE'] >= self.start)]
        return df_raw_data
    ...
    def __call__(self, df_raw_data, factor=None, fillna=True, win=True, stand=True, dropstyle=True,
        ...
        factor = factor or self.factor_name_ls[0]
        if merge_cap:
            df_raw_data = self.load_data(df_raw_data)
        else:
            df_raw_data = df_raw_data
        df_columns = df_raw_data.columns.drop(self.factor_name_ls).to_list() + [factor]
        ...
        return df_processed[['TICKER','DATE',factor_name]].reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    for j in range(len(file_name_list)):
        ...
        try:
            ...
            print(out_put_name,df_processed.DATE.max())
