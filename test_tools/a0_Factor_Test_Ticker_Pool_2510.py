...
class Factor_Test():
    ...
    def load_Data(self):
        ...
        self.price_daily = copy.deepcopy(self.price)
        self.price_daily['Return_daily'] = self.price_daily.groupby('TICKER')['price'].shift(-shift_date - 1).reset_index(
            drop=True) / self.price_daily.groupby('TICKER')['price'].shift(-shift_date).reset_index(drop=True)

        self.price_daily['Return_daily'] = (self.price_daily['Return_daily'] - 1) * 100
        self.price_daily = self.price_daily[['TICKER', 'DATE', 'Return_daily']]
    ...
    def load_factor_data(self, factor_path):
        ...
        all_data = pd.merge(daily_Data, all_data, how='right', on=['TICKER', 'DATE']).reset_index(drop=True)
        factor_name = list(np.setdiff1d(all_data.columns, ['TICKER', 'DATE', 'Return','Return_daily']))[0]
    ...
    def calc_Factors_metrics(self, factor_data, factor_name, bin_num, plot_save_path):
        # region 计算因子指标
        factor_data = factor_data.dropna(subset=['Return','Return_daily']).reset_index(drop=True)
        ic_results = factor_data.groupby('DATE').apply(self.cal_factors_validation,
                                                       factor_name=factor_name).reset_index(drop=True)
        date_period = self.date_shift
        datels = sorted(list(factor_data['DATE'].unique()))
        trade_date = [datels[i] for i in range(0,len(datels),date_period)]
        factor_data['trade_date'] = factor_data['DATE'].apply(lambda x:max([d for d in trade_date if d <= x]))

        factor_trade_date = factor_data[factor_data['DATE'].isin(trade_date)]
        bins = {}
        for key, series in tqdm(factor_trade_date.set_index('TICKER').groupby('DATE')[factor_name],
        ...
        factor_data = factor_data.sort_values(by=['TICKER','DATE']).reset_index(drop=True)
        factor_data['bins'] = factor_data.groupby('TICKER')['bins'].ffill()

        factor_data['exchange_date'] = factor_data['trade_date']

        # split_by_date = factor_data.groupby(['exchange_date', 'bins']).agg({'Return': 'mean'}).reset_index(drop=False)
        ...
        split_by_date = factor_data.groupby(['DATE', 'bins']).agg({'Return_daily': 'mean'}).reset_index(drop=False)
        split_by_date['Return_daily'] = split_by_date['Return_daily'] / 100 + 1
        split_by_date['Return_cumprod'] = split_by_date.groupby('bins')['Return_daily'].cumprod().reset_index(drop=True)
        pvt_split_by_date = pd.pivot_table(split_by_date, columns=['bins'], index=['DATE'], values='Return_daily')
        ...
        if mean_ic > 0:
            ...
            long_side = factor_data[factor_data['bins'] == bin_num][['Return_daily', factor_name, 'DATE']]
        else:
            pvt_split_by_date['long_short_Return'] = pvt_split_by_date[1] / pvt_split_by_date[bin_num]
            pvt_split_by_date['long_short_Return'] = abs(pvt_split_by_date['long_short_Return'])
            long_side = factor_data[factor_data['bins'] == 1][['Return_daily', factor_name, 'DATE']]

        longside_metrics = long_side.groupby('DATE').apply(self.cal_factors_validation,
                                                           factor_name=factor_name).reset_index(drop=True)
        longside_metrics_ic = longside_metrics['Return_daily'].mean()
        longside_metrics_ic_std = longside_metrics['Return_daily'].std()
        ...
        for trade_date, df_date in tqdm(df_factor.groupby('DATE'), desc='计算各层的平均收益'):
            for bin in range(1, bin_num + 1):
                ...
                return_data = df_temp['Return_daily']
        ...
        bins_df_IC = {}
        bins_df_Rank_IC = {}
        for bin, df_temp1 in tqdm(factor_data.drop_duplicates(subset=['TICKER','trade_date'],keep='first').groupby('bins'), desc=f'{factor_name}因子计算IR并绘图'):
            IC_series = pd.Series()
        ...
        color_list = [mcolors.to_hex(cm.get_cmap("RdYlBu", 10)(i)) for i in range(10)]
...
if __name__ == '__main__':
    ...
    param_list = getMultiInput('请输入参数',
                               ['因子测试起始日期(默认20220104)', '因子测试结束日期(默认20250919)', '因子文件路径',
                                '因子目标未来天数'])
    ...
    for factor_path in ls:
        try:
            ...
            plt_save_path = rf'\\Desktop-79nue61\因子测试结果\{researcher}\{today}_{use_market}_{start_date}_{end_date}_{aim_Date}\{factor_path[:-8]}'
    ...
    # results_path = rf'\\Desktop-79nue61\因子测试结果\{researcher}\{today}_{use_market}_{start_date}_{end_date}_{aim_Date}'
    # results_path = rf'\\192.168.1.210\因子测试结果\{researcher}\{today}_{use_market}_{start_date}_{end_date}_{aim_Date}'
    results_path = rf'\\192.168.1.210\因子测试结果\{researcher}\{today}_{use_market}_{start_date}_{end_date}_{aim_Date}'
    ls = os.listdir(results_path)
    res = []
    for f in ls:
        now = os.listdir(os.path.join(results_path,f))
        now = [i for i in now if i.endswith('.xlsx')]
        if len(now) > 0:
            now = pd.read_excel(os.path.join(results_path,f,now[0]))
            res.append(now)
        else:
            pass
    res = pd.concat(res).reset_index(drop=True)
    res.to_excel(os.path.join(results_path,'因子测试结果.xlsx'), index=False)